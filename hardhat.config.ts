...
// Normalize private keys from env with or without 0x prefix
function normalizePk(pk?: string): string | undefined {
  if (!pk) return undefined;
  return pk.startsWith("0x") ? pk : `0x${pk}`;
}

// Accept PRIVATE_KEY or SEPOLIA_PRIVATE_KEY for live networks, and LOCAL_PRIVATE_KEY for localhost
const PRIVATE_KEY: string | undefined = normalizePk(process.env.PRIVATE_KEY || process.env.SEPOLIA_PRIVATE_KEY);
const LOCAL_PRIVATE_KEY: string | undefined = normalizePk(process.env.LOCAL_PRIVATE_KEY);
...
const config: HardhatUserConfig = {
  ...
  networks: {
    ...
    localhost: {
      ...
      chainId: 31337,
      // Use LOCAL_PRIVATE_KEY if provided, otherwise let local node manage accounts
